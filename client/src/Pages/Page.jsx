...
function Page() {
  ...
  const handleTrim = async () => {
    ...
    try {
      const prompt = `Rewrite the following social media post to be engaging, professional, and strictly under 280 characters. 
      Maintain the original meaning and tone. 
      Add 3-5 relevant hashtags at the end. 
      Do not include quotes around the output.
      
      Original Post: "${inputText}"`;
      ...
      if (!response.ok) {
        throw new Error('Failed to trim text');
      }

      const data = await response.json();
      const generatedText = data.candidates?.[0]?.content?.parts?.[0]?.text || "Could not generate text.";
      setTrimmedText(generatedText.trim());
          ...
          <div className="output-section">
            <div className="output-header">
              ...
              <div className="output-badge">
                <span className={`char-counter ${trimmedText.length <= 280 ? 'char-count-success' : 'char-count-error'}`} style={{ fontSize: '0.75rem' }}>
